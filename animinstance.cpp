...
animinstance::animinstance(std::shared_ptr<animmodel> model, glm::vec3 worldpos, bool randomize){
...
void animinstance::resetnodedata() {
	mgltfmodel->resetnodedata(mrootnode);
	updatenodematrices(mrootnode);
}

std::shared_ptr<vkmesh> animinstance::getskeleton(){
	mskeletonmesh->verts.clear();
	getskeletonpernode(mrootnode->getchildren().at(0));
	return mskeletonmesh;
}

void animinstance::getskeletonpernode(std::shared_ptr<vknode> treenode){
...
void animinstance::updatenodematrices(std::shared_ptr<vknode>treenode) {
...
void animinstance::updatejointmatrices(std::shared_ptr<vknode>treenode) {
...
void animinstance::updatejointdualquats(std::shared_ptr<vknode>treenode) {
...
int animinstance::getjointmatrixsize() {
	return mjointmats.size();
}

std::vector<glm::mat4> animinstance::getjointmats() {
	return mjointmats;
}
int animinstance::getjointdualquatssize() {
	return mjointdualquats.size();
}
std::vector<glm::mat2x4> animinstance::getjointdualquats() {
	return mjointdualquats;
}



void animinstance::checkforupdates() {
...
void animinstance::updateanimation() {
...
void animinstance::playanimation(int animNum, float speedDivider, float blendFactor, replaydirection direction) {
...
void animinstance::playanimation(int sourceAnimNumber, int destAnimNumber, float speedDivider, float blendFactor, replaydirection direction) {
	double currentTime = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count();
...
void animinstance::blendanimationframe(int animNum, float time, float blendFactor) {
	manimclips.at(animNum)->blendFrame(mnodelist, madditiveanimationmask, time,
		blendFactor);
	updatenodematrices(mrootnode);
}

void animinstance::crossblendanimationframe(int sourceAnimNumber, int destAnimNumber,
...
void animinstance::updateadditivemask(std::shared_ptr<vknode> treeNode, int splitNodeNum) {
	/* break chain here */
...
void animinstance::setskeletonsplitnode(int nodeNum) {
	std::fill(madditiveanimationmask.begin(), madditiveanimationmask.end(), true);
...
void animinstance::setinstancesettings(modelsettings& settings) {
	mmodelsettings = settings;
}

modelsettings& animinstance::getinstancesettings() {
...
glm::vec3 animinstance::getwpos() {
	return mmodelsettings.msworldpos;
}

glm::quat animinstance::getwrot() {
...
float animinstance::getanimendtime(int animNum) {
	return manimclips.at(animNum)->getEndTime();
}

void animinstance::setinversekindematicsnode(int effectorNodeNum, int ikChainRootNodeNum) {
...
void animinstance::setnumikiterations(int iterations) {
	miksolver.setnumiterations(iterations);
}

glm::vec3* animinstance::getinstpos(){
	return &mmodelsettings.msworldpos;
}

void animinstance::solveikbyccd(glm::vec3 target) {
	miksolver.solveccd(target);
	updatenodematrices(miksolver.getikchainrootnode());
}

void animinstance::solveikbyfabrik(glm::vec3 target) {
