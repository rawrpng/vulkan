...
void texmodel::createvertexbuffers(vkobjs& objs) {
    ...
    vkvbo::init(objs, mgltfobjs.rdgltfvertexbufferdata.at(0).at(0).at(1), mmodel.size() * sizeof(glm::vec2));
}
...
void texmodel::uploadvertexbuffers(vkobjs& objs) {

    for (int i{ 0 }; i < mgltfobjs.rdgltfvertexbufferdata.size(); ++i) {
        for (int j{ 0 }; j < mgltfobjs.rdgltfvertexbufferdata.at(i).size(); ++j) {
            vkvbo::upload(objs, mgltfobjs.rdgltfvertexbufferdata.at(i).at(j).at(0), mmodel);
...
void texmodel::uploadindexbuffers(vkobjs& objs) {
